import { JSDOM } from 'jsdom';

export interface CodeBlock {
  language: string;
  code: string;
}

export interface Command {
  action: string;
  params: Record<string, string>;
}

export interface ParsedResponse {
  text: string;
  codeBlocks: CodeBlock[];
  commands: Command[];
}

export interface IResponseParser {
  parse(html: string): ParsedResponse;
}

export class ResponseParser implements IResponseParser {
  public parse(html: string): ParsedResponse {
    const dom = new JSDOM(`<body>${html}</body>`).window.document;
    const text = Array.from(dom.querySelectorAll('p, li'))
      .map(e => e.textContent?.trim() || '')
      .filter(Boolean)
      .join('\n\n');
    const codeBlocks: CodeBlock[] = Array.from(dom.querySelectorAll('pre > code'))
      .map(e => {
        const m = e.className.match(/language-(\w+)/);
        return { language: m?.[1] || '', code: e.textContent || '' };
      });
    const combined = text + codeBlocks.map(cb => `\n\n\`\`\`${cb.language}\n${cb.code}\n\`\`\``).join('');
    return { text, codeBlocks, commands: this.parseCommands(combined) };
  }

  private parseCommands(str: string): Command[] {
    const cmds: Command[] = [];
    const re = /\[VSCODE_COMMAND:(\w+)=['"]([^'"]+)['"]([^]*?)\]/g;
    let m;
    while ((m = re.exec(str))) {
      const [, action, payload, rest = ''] = m;
      const params: Record<string, string> = { arg: payload };
      if (action === 'writeFile') { params.path = payload; delete params.arg; }
      const kvRe = /\b(\w+)='([^']*)'/g;
      let kv;
      while ((kv = kvRe.exec(rest))) { params[kv[1]] = kv[2]; }
      cmds.push({ action, params });
    }
    return cmds;
  }
}
